...
public class TurtlePond implements KeyEventDispatcher {
	...
	int cookieX = new Random().nextInt(200);
	int cookieY = new Random().nextInt(200);
	...
	void setup() throws Exception {
		...
		JOptionPane.showMessageDialog(null, "Move the turtle with the arrow keys to find the hidden cookie!!! You'll get warmer the closer you get!!! Hurry or she'll die of hunger!!!");
	...
	private void checkForFood() throws Exception {
		...
		if (Math.abs(tortoiseLocationX-cookieX) <5){
			JOptionPane.showMessageDialog(null, "YOU FOUND THE COOKIE AND SAVED THE TORTOISE!!!");
		}
	...
	if (Math.abs(tortoiseLocationX-cookieX) >20){
		...
		if(getTimeElapsed()>20){
			JOptionPane.showMessageDialog(null, "THE TIME RAN OUT!!! YOUR TORTOISE DIED OF HUNGER AND IT WAS YOUR FAULT!!!");
		}
		if(wasHereBefore(tortoiseLocationX,tortoiseLocationY)== true){
			JOptionPane.showMessageDialog(null, "YOU FAILED!!! YOU STEPPED ON YOUR OWN PATH!!!");
			//System.exit(0);
		}
		}
	}
